...
class Uploader
{
     ...
     */
    public function __construct( $fileField , $config , $prefix = '',$model=FALSE, $base64 = false )
    {
...
//         }
        ...
        if ( $this->stateInfo == $this->stateMap[ 0 ] ) {
            ...
            $this->img_url = 'http://'.$_SERVER['HTTP_HOST'].Yii::getAlias('@web/image').'/'.$this->fileName ; //存储路径
        }
    ...
    private function upFiles()
    {
        ...
        foreach ($file['name'] as $key=>$v_name)
        {
            if ( !$v_name ) {
                $this->stateInfo = $this->getStateInfo( 'POST' );
                return;
            ...
            if ( $file[ 'error' ][$key] ) {
                $this->stateInfo = $this->getStateInfo( $file[ 'error' ][$key] );
                return;
            }
            $this->oriName = $file[ 'name' ][$key];
            $this->fileSize = $file[ 'size' ][$key];
            $this->tmp_name = $file[ 'tmp_name' ][$key];
            $this->file[ "name" ] = $this->oriName ;
            $this->fileType = $this->getFileExt();
            if ( !$this->checkSize() ) {
                $this->stateInfo = $this->getStateInfo( "SIZE" );
                return;
            ...
            $this->fullName = $folder . '/' . $this->getName();
            if ( $this->stateInfo == $this->stateMap[ 0 ] ) {
                if ( !move_uploaded_file( $file[ "tmp_name" ][$key] , $this->fullName ) ) {
                    $this->stateInfo = $this->getStateInfo( "MOVE" );
                }
                $this->img_url = 'http://'.$_SERVER['HTTP_HOST'].Yii::getAlias('@web/image').'/'.$this->fileName ; //存储路径
            }
            $this->reqdata[] = $this->getFileInfo();
        }
        return;
